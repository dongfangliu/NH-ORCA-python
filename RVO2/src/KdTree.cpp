...
namespace RVO {
	...
	void KdTree::buildObstacleTree()
	{
		...
		std::vector<std::shared_ptr<Obstacle>> obstacles(sim_->obstacles_.size());
	...
	KdTree::ObstacleTreeNode *KdTree::buildObstacleTreeRecursive( std::vector<std::shared_ptr<Obstacle>> &obstacles)
	{
		if (obstacles.empty()) {
			return NULL;
		}
			...
			for (size_t i = 0; i < obstacles.size(); ++i) {
				...
				std::shared_ptr<Obstacle> obstacleI1 = obstacles[i];
				std::shared_ptr<Obstacle> obstacleI2 = obstacleI1->nextObstacle_;
				...
				for (size_t j = 0; j < obstacles.size(); ++j) {
					...
					std::shared_ptr<Obstacle> obstacleJ1 = obstacles[j];
					std::shared_ptr<Obstacle> obstacleJ2 = obstacleJ1->nextObstacle_;

					 float j1LeftOfI = leftOf(obstacleI1->point_, obstacleI2->point_, obstacleJ1->point_);
					 float j2LeftOfI = leftOf(obstacleI1->point_, obstacleI2->point_, obstacleJ2->point_);
			...
			/* Build split node. */
			std::vector<std::shared_ptr<Obstacle>> leftObstacles(minLeft);
			std::vector<std::shared_ptr<Obstacle>> rightObstacles(minRight);

			size_t leftCounter = 0;
			size_t rightCounter = 0;
			 size_t i = optimalSplit;
			...
			for (size_t j = 0; j < obstacles.size(); ++j) {
				...
				if (j1LeftOfI >= -RVO_EPSILON && j2LeftOfI >= -RVO_EPSILON) {
					...
					/* Split obstacle j. */
					 float t = det(obstacleI2->point_ - obstacleI1->point_, obstacleJ1->point_ - obstacleI1->point_) / det(obstacleI2->point_ - obstacleI1->point_, obstacleJ1->point_ - obstacleJ2->point_);

					 Eigen::Vector2f splitpoint = obstacleJ1->point_ + t * (obstacleJ2->point_ - obstacleJ1->point_);

					std::shared_ptr<Obstacle> newObstacle = std::make_shared<Obstacle>();
	...
	void KdTree::computeAgentNeighbors(std::shared_ptr<Agent>agent, float &rangeSq) 
	{
		queryAgentTreeRecursive(agent, rangeSq, 0);
	}

	void KdTree::computeObstacleNeighbors(std::shared_ptr<Agent>agent, float rangeSq) 
	{
	...
	void KdTree::queryAgentTreeRecursive(std::shared_ptr<Agent>agent, float &rangeSq, size_t node) 
	{
	...
	void KdTree::queryObstacleTreeRecursive(std::shared_ptr<Agent>agent, float rangeSq,  ObstacleTreeNode *node) 
	{
		if (node == NULL) {
			return;
		}
		else {
			std::shared_ptr<Obstacle> obstacle1 = node->obstacle;
			std::shared_ptr<Obstacle> obstacle2 = obstacle1->nextObstacle_;

			 float agentLeftOfLine = leftOf(obstacle1->point_, obstacle2->point_, agent->position_);

			queryObstacleTreeRecursive(agent, rangeSq, (agentLeftOfLine >= 0.0f ? node->left : node->right));

			 float distSqLine = sqr(agentLeftOfLine) / absSq(obstacle2->point_ - obstacle1->point_);
	...
	bool KdTree::queryVisibility( Eigen::Vector2f &q1,  Eigen::Vector2f &q2, float radius) 
	{
		return queryVisibilityRecursive(q1, q2, radius, obstacleTree_);
	}

	bool KdTree::queryVisibilityRecursive( Eigen::Vector2f &q1,  Eigen::Vector2f &q2, float radius,  ObstacleTreeNode *node) 
	{
		if (node == NULL) {
			return true;
		}
			 ...
			 float q1LeftOfI = leftOf(obstacle1->point_, obstacle2->point_, q1);
			 float q2LeftOfI = leftOf(obstacle1->point_, obstacle2->point_, q2);
			 float invLengthI = 1.0f / absSq(obstacle2->point_ - obstacle1->point_);
			...
			if (q1LeftOfI >= 0.0f && q2LeftOfI >= 0.0f) {
				...
				return queryVisibilityRecursive(q1, q2, radius, node->left) && queryVisibilityRecursive(q1, q2, radius, node->right);
				 ...
				 float point1LeftOfQ = leftOf(q1, q2, obstacle1->point_);
				 float point2LeftOfQ = leftOf(q1, q2, obstacle2->point_);
