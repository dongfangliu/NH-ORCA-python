...
namespace RVO {
	 ...
	 */
	//  size_t RVO_ERROR = std::numeric_limits<size_t>::max();
	...
	public:
		/**
		 * \brief      ructs a simulator instance.
		 */
		RVOSimulator();

		/**
		 * \brief      ructs a simulator instance and sets the default
		...
		RVOSimulator(float timeStep, float neighborDist, size_t maxNeighbors,
					 ...
					 float maxSpeed,  Eigen::Vector2f &velocity );
		 ...
		 *                             this agent.
		 * \return     The number of the agent, or RVO::std::numeric_limits<size_t>::max() when the agent
		 *             defaults have not been set.
		 */
		size_t addAgent( Eigen::Vector2f &position);
		 ...
		 */
		size_t addAgent( Eigen::Vector2f &position, float neighborDist,
		 ...
		 *             or RVO::std::numeric_limits<size_t>::max() when the number of vertices is less than two.
		 * \note       To add a "negative" obstacle, e.g. a bounding polygon around
		 *             the environment, the vertices should be listed in clockwise
		 *             order.
		 */
		size_t addObstacle( std::vector<Eigen::Vector2f> &vertices);
		...
		/**
		 * \brief      Returns the count of ORCA raints used to compute
		 *             the current velocity for the specified agent.
		 * \param      agentNo         The number of the agent whose count of ORCA
		 *                             raints is to be retrieved.
		 * \return     The count of ORCA raints used to compute the current
		...
		/**
		 * \brief      Returns the specified ORCA raint of the specified
		 *             agent.
		 * \param      agentNo         The number of the agent whose ORCA
		 *                             raint is to be retrieved.
		 * \param      lineNo          The number of the ORCA raint to be
		 *                             retrieved.
		 * \return     A line representing the specified ORCA raint.
		 * \note       The halfplane to the left of the line is the region of
		 *             permissible velocities with respect to the specified
		 *             ORCA raint.
		 */
		 Line &getAgentORCALine(size_t agentNo, size_t lineNo) ;
		 ...
		 *             agent.
		 */
		 Eigen::Vector2f &getAgentPosition(size_t agentNo) ;
		 ...
		 */
		 Eigen::Vector2f &getAgentPrefVelocity(size_t agentNo) ;
		 ...
		 */
		 Eigen::Vector2f &getAgentVelocity(size_t agentNo) ;
		 ...
		 *             vertex.
		 */
		 Eigen::Vector2f &getObstacleVertex(size_t vertexNo) ;
		 ...
		 */
		size_t getNextObstacleVertexNo(size_t vertexNo) ;
		 ...
		 *             obstacle vertex in its polygon.
		 */
		size_t getPrevObstacleVertexNo(size_t vertexNo) ;
		 ...
		 *             processed.
		 */
		bool queryVisibility( Eigen::Vector2f &point1,  Eigen::Vector2f &point2,
							 float radius = 0.0f) ;
		 ...
		 *                             position.
		 */
		void setAgentPosition(size_t agentNo,  Eigen::Vector2f &position);
		 ...
		 *                             preferred velocity.
		 */
		void setAgentPrefVelocity(size_t agentNo,  Eigen::Vector2f &prefVelocity);
		 ...
		 *                             velocity.
		 */
		void setAgentVelocity(size_t agentNo,  Eigen::Vector2f &velocity);
	...
	private:
		...
		std::shared_ptr<Agent> defaultAgent_=nullptr;
		float globalTime_;
